...
pub fn overlapping_pairs(file_path: &String) -> usize {
    let file_contents =
        fs::read_to_string(file_path).expect("Something went wrong reading the file");
    ...
    section_assignments
        .chunks(2)
        .filter(|assignment_pair| is_overlapping(assignment_pair))
        .count()
}
...
fn is_overlapping(assignment_pair: &&[(u64, u64)]) -> bool {
    let mut assigment_pair_iter = assignment_pair.iter();
    ...
    if first_assignment_start < second_assignment_start {
        if first_assignment_end >= second_assignment_start {
            return true;
        }
    } else {
        if second_assignment_end >= first_assignment_start {
            return true;
        }
    }
