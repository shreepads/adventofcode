...
impl Number {
    ...
    pub fn add(&mut self, sfish_no: &Number) {
        ...
        for node in self.nodes.iter_mut() {
            if node.depth != 0 {
                // don't increase for orphans or old roots
                node.depth += 1;
            }
        }
    ...
    fn split(&mut self, split_id: usize) {
        ...
        if self.nodes[old_parent_id].part1_id == Some(split_id) {
            ...
            println!(
                "Old parent {} doesn't have split id {} as child",
                old_parent_id, split_id
            )
        }
    ...
    fn replace_with_zero(&mut self, explode_id: usize) {
        ...
        if parent_node.part1_id == Some(explode_id) {
            ...
            // error
            println!(
                "Explode id {} parent doesn't point to explode id",
                explode_id
            );
        }
...
mod tests {
    ...
    fn magnitude_3() {
        let mut sno =
            Number::new("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]".to_string());
        assert_eq!(3488, sno.magnitude());
    }

    #[test]
    fn magnitude_4() {
        let mut sno = Number::new(
